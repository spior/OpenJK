...
void QDECL PrintMsg( gentity_t *ent, const char *fmt, ... ) {
	...
	if (vsprintf (msg, fmt, argptr) > sizeof(msg)) {
		gi.Error( ERR_DROP, "PrintMsg overrun" );
	}
	va_end (argptr);
	...
	gi.SendServerCommand ( ( (ent == NULL) ? -1 : ent-g_entities ), va("print \"%s\"", msg ));
}
*/
...
void Team_FragBonuses(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker)
{
	...
	if ( ( ( VectorLength(v1) < CTF_TARGET_PROTECT_RADIUS &&
		gi.InPVS(flag->r.currentOrigin, targ->r.currentOrigin ) ) ||
		( VectorLength(v2) < CTF_TARGET_PROTECT_RADIUS &&
		gi.InPVS(flag->r.currentOrigin, attacker->r.currentOrigin ) ) ) &&
	...
	if (carrier && carrier != attacker) {
		...
		if ( ( ( VectorLength(v1) < CTF_ATTACKER_PROTECT_RADIUS &&
			gi.InPVS(carrier->r.currentOrigin, targ->r.currentOrigin ) ) ||
			( VectorLength(v2) < CTF_ATTACKER_PROTECT_RADIUS &&
				gi.InPVS(carrier->r.currentOrigin, attacker->r.currentOrigin ) ) ) &&
			attacker->client->sess.sessionTeam != targ->client->sess.sessionTeam) {
...
void Team_ReturnFlagSound( gentity_t *ent, int team ) {
	...
	if (ent == NULL) {
		gi.Print ("Warning:  NULL passed to Team_ReturnFlagSound\n");
		return;
	}
...
void Team_TakeFlagSound( gentity_t *ent, int team ) {
	...
	if (ent == NULL) {
		gi.Print ("Warning:  NULL passed to Team_TakeFlagSound\n");
		return;
	}
...
void Team_CaptureFlagSound( gentity_t *ent, int team ) {
	...
	if (ent == NULL) {
		gi.Print ("Warning:  NULL passed to Team_CaptureFlagSound\n");
		return;
	}
...
gentity_t *Team_GetLocation(gentity_t *ent)
{
	...
	for (eloc = level.locationHead; eloc; eloc = eloc->nextTrain) {
		...
		if ( !gi.InPVS( origin, eloc->r.currentOrigin ) ) {
			continue;
		}
