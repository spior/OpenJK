...
void TeleportPlayer( gentity_t *player, vec3_t origin, vec3_t angles ) {
	...
	gi.UnlinkEntity ((sharedEntity_t *)player);
	...
	if ( player->client->sess.sessionTeam != TEAM_SPECTATOR ) {
		gi.LinkEntity ((sharedEntity_t *)player);
	}
}
...
void misc_model_breakable_gravity_init( gentity_t *ent, qboolean dropToFloor )
{
	...
	if ( dropToFloor )
	{
		...
		gi.Trace( &tr, top, ent->r.mins, ent->r.maxs, bottom, ent->s.number, MASK_NPCSOLID, qfalse, 0, 0 );
...
void SP_misc_G2model( gentity_t *ent ) {
	...
	gi.G2API_InitGhoul2Model(&ent->s, name1, G_ModelIndex( name1 ), 0, 0, 0, 0);
	gi.G2API_SetBoneAnim(ent->s.ghoul2, 0, "model_root", 0, 12, BONE_ANIM_OVERRIDE_LOOP, 1.0f, level.time, -1, -1);
	ent->s.radius = 150;
//	VectorSet (ent->r.mins, -16, -16, -16);
//	VectorSet (ent->r.maxs, 16, 16, 16);
	gi.LinkEntity ((sharedEntity_t *)ent);
...
void locateCamera( gentity_t *ent ) {
	...
	if ( !owner ) {
		gi.Print( "Couldn't find target for misc_partal_surface\n" );
		G_FreeEntity( ent );
		return;
	}
...
void SP_misc_bsp(gentity_t *ent) 
{
	...
	gi.SetBrushModel( (sharedEntity_t *)ent, temp );  // SV_SetBrushModel -- sets mins and maxs
	G_BSPIndex(temp);
	...
	*/
	...
	gi.SetActiveSubBSP(ent->s.modelindex);
...
void SP_terrain(gentity_t *ent) 
{
	...
	gi.SetBrushModel( (sharedEntity_t *)ent, ent->model );
...
//	shaderNum = gi.CM_GetShaderNum(s.modelindex);
	...
	if (RMG.integer)
	{
		...
		gi.Cvar_VariableStringBuffer("RMG_terrain", temp, MAX_QPATH);
		Com_sprintf(final, MAX_QPATH, "%s", temp);
		AddSpawnField("terrainDef", temp);
 
		gi.Cvar_VariableStringBuffer("RMG_instances", temp, MAX_QPATH);
		Com_sprintf(final, MAX_QPATH, "%s", temp);
		AddSpawnField("instanceDef", temp);

		gi.Cvar_VariableStringBuffer("RMG_miscents", temp, MAX_QPATH);
		Com_sprintf(final, MAX_QPATH, "%s", temp);
...
//	SetIsRandom(common->GetIsRandom());
	...
	if ( RMG.integer ) 
	{
		...
		if ( level.gametypeData->teams )
		{
			...
			gi.GetConfigstring ( CS_GAMETYPE_REDTEAM, temp, MAX_QPATH );
			if ( Q_stricmp ( temp, level.gametypeTeam[TEAM_RED] ) )
			{
				level.gametypeTeam[TEAM_RED] = gi.VM_LocalStringAlloc ( temp );
			}

			// Blue team change from RMG ?
			gi.GetConfigstring ( CS_GAMETYPE_BLUETEAM, temp, MAX_QPATH );
			if ( Q_stricmp ( temp, level.gametypeTeam[TEAM_BLUE] ) )
			{
				level.gametypeTeam[TEAM_BLUE] = gi.VM_LocalStringAlloc ( temp );
			}
		}
		*/
	}
...
void G_PortalifyEntities(gentity_t *ent)
{
	...
	while (i < MAX_GENTITIES)
	{
		...
		if (scan && scan->inuse && scan->s.number != ent->s.number && gi.InPVS(ent->s.origin, scan->r.currentOrigin))
		{
			trace_t tr;

			gi.Trace(&tr, ent->s.origin, vec3_origin, vec3_origin, scan->r.currentOrigin, ent->s.number, CONTENTS_SOLID, qfalse, 0, 0);
...
void SP_misc_skyportal (gentity_t *ent)
{
	...
	gi.SetConfigstring( CS_SKYBOXORG, va("%.2f %.2f %.2f %.1f %i %.2f %.2f %.2f %i %i", ent->s.origin[0], ent->s.origin[1], ent->s.origin[2], fov_x, (int)isfog, fogv[0], fogv[1], fogv[2], fogn, fogf ) );
...
void HolocronTouch(gentity_t *self, gentity_t *other, trace_t *trace)
{
	...
	{ //once we pick up this holocron we'll have all of them, so give us super special best prize!
		//gi.Print("You deserve a pat on the back.\n");
	}
...
void SP_misc_holocron(gentity_t *ent)
{
	...
	gi.Trace( &tr, ent->s.origin, ent->r.mins, ent->r.maxs, dest, ent->s.number, MASK_SOLID, qfalse, 0, 0 );
	if ( tr.startsolid )
	{
		gi.Print ("SP_misc_holocron: misc_holocron startsolid at %s\n", vtos(ent->s.origin));
		G_FreeEntity( ent );
		return;
	}
...
void SP_misc_ammo_floor_unit(gentity_t *ent)
{
	...
	if ( tr.startsolid )
	{
		gi.Print ("SP_misc_ammo_floor_unit: misc_ammo_floor_unit startsolid at %s\n", vtos(ent->s.origin));
		G_FreeEntity( ent );
		return;
	}
...
void SP_misc_shield_floor_unit( gentity_t *ent )
{
	...
	if ( tr.startsolid )
	{
		gi.Print ("SP_misc_shield_floor_unit: misc_shield_floor_unit startsolid at %s\n", vtos(ent->s.origin));
		G_FreeEntity( ent );
		return;
	}
...
void maglock_link( gentity_t *self )
{
	...
	gi.Trace( &trace, start, vec3_origin, vec3_origin, end, self->s.number, MASK_SHOT, qfalse, 0, 0 );
...
void misc_faller_create( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	...
	gi.LinkEntity((sharedEntity_t *)faller);
}
...
//Maybe I should use my gi.TrueMalloc/gi.TrueFree stuff with this.
