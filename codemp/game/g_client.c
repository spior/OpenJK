...
//	gentity_t *te;
	...
	gi.SetConfigstring ( CS_CLIENT_JEDIMASTER, va("%i", other->s.number ) );
...
gentity_t *SelectRandomFurthestSpawnPoint ( vec3_t avoidPoint, vec3_t origin, vec3_t angles, team_t team ) {
	...
	{//couldn't find any of the above
		...
		if (!numSpots) {
			...
			if (!spot)
				gi.Error( ERR_DROP, "Couldn't find a spawn point" );
...
static qboolean CopyToBodyQue( gentity_t *ent ) {
	...
	contents = gi.PointContents( ent->s.origin, -1 );
	...
	gi.UnlinkEntity ((sharedEntity_t *)body);
	body->s = ent->s;
	...
	gi.SendServerCommand(-1, va("ircg %i %i %i %i", ent->s.number, body->s.number, body->s.weapon, islight));
	...
	gi.LinkEntity ((sharedEntity_t *)body);
...
{ //do whatever should be done taking ragdoll and dismemberment states into account.
	...
	{ //bodyque func didn't manage to call ircg so call this to assure our limbs and ragdoll states are proper on the client.
		gi.SendServerCommand(-1, va("rcg %i", ent->s.clientNum));
	}
}
...
void G_DebugWrite(const char *path, const char *text)
{
	...
	gi.FS_Open( path, &f, FS_APPEND );
	gi.FS_Write(text, strlen(text), f);
	gi.FS_Close(f);
}
#endif
...
qboolean G_SaberModelSetup(gentity_t *ent)
{
	...
	while (i < MAX_SABERS)
	{
		if (ent->client->saber[i].model[0])
		{
			...
			{
				gi.G2API_CleanGhoul2Models(&(ent->client->weaponGhoul2[i]));
			}
			gi.G2API_InitGhoul2Model(&ent->client->weaponGhoul2[i], ent->client->saber[i].model, 0, 0, -20, 0, 0);
			...
			if (ent->client->weaponGhoul2[i])
			{
				...
				{
					gi.G2API_SetSkin(ent->client->weaponGhoul2[i], 0, ent->client->saber[i].skin, ent->client->saber[i].skin);
				}

				if (ent->client->saber[i].saberFlags & SFL_BOLT_TO_WRIST)
				{
					gi.G2API_SetBoltInfo(ent->client->weaponGhoul2[i], 0, 3+i);
				}
				else
				{ // bolt to right hand for 0, or left hand for 1
					gi.G2API_SetBoltInfo(ent->client->weaponGhoul2[i], 0, i);
				}
				...
				while (j < ent->client->saber[i].numBlades)
				{
					...
					tagBolt = gi.G2API_AddBolt(ent->client->weaponGhoul2[i], 0, tagName);

					if (tagBolt == -1)
					{
						if (j == 0)
						{ //guess this is an 0ldsk3wl saber
							tagBolt = gi.G2API_AddBolt(ent->client->weaponGhoul2[i], 0, "*flash");
				...
				gi.G2API_CopySpecificGhoul2Model(ent->client->weaponGhoul2[i], 0, ent->ghoul2, i+1); 
			}
		}
		else
		{
...
void SetupGameGhoul2Model(gentity_t *ent, char *modelname, char *skinName)
{
	...
	if (!precachedKyle)
	{
		...
		handle = gi.G2API_InitGhoul2Model(&precachedKyle, afilename, 0, 0, -20, 0, 0);
		...
		defSkin = gi.R_RegisterSkin("models/players/kyle/model_default.skin");
		gi.G2API_SetSkin(precachedKyle, 0, defSkin, defSkin);
	}
	...
	if (precachedKyle && gi.G2API_HaveWeGhoul2Models(precachedKyle))
	{
		...
		{ //rww - allow option for perplayer models on server for collision and bolt stuff.
			...
			if ( ent->client->NPC_class == CLASS_VEHICLE )
			{
				...
				{
					skinHandle = gi.R_RegisterSkin(va("models/players/%s/model_%s.skin", modelname, ent->m_pVehicle->m_pVehicleInfo->skin));
				}
				else
				{
					skinHandle = gi.R_RegisterSkin(va("models/players/%s/model_default.skin", modelname));
				}
			}
			else
			{
			...
			if (skin[0])
			{
				...
				skinHandle = gi.R_RegisterSkin(useSkinName);
			}

			strcpy(modelFullPath, va("models/players/%s/model.glm", truncModelName));
			handle = gi.G2API_InitGhoul2Model(&ent->ghoul2, modelFullPath, 0, skinHandle, -20, 0, 0);
			...
			{ //Huh. Guess we don't have this model. Use the default.
				...
				ent->ghoul2 = NULL;
				gi.G2API_DuplicateGhoul2Instance(precachedKyle, &ent->ghoul2);
			}
			else
			{
				gi.G2API_SetSkin(ent->ghoul2, 0, skinHandle, skinHandle);

				GLAName[0] = 0;
				gi.G2API_GetGLAName( ent->ghoul2, 0, GLAName);
	...
	gi.G2API_AttachInstanceToEntNum(ent->ghoul2, ent->s.number, qtrue);
	...
	{ //do special vehicle stuff
		...
		i = gi.G2API_AddBolt( ent->ghoul2, 0, "model_root" );

		// Setup the droid unit.
		ent->m_pVehicle->m_iDroidUnitTag = gi.G2API_AddBolt( ent->ghoul2, 0, "*droidunit" );

		// Setup the Exhausts.
		for ( i = 0; i < MAX_VEHICLE_EXHAUSTS; i++ )
		{
			Com_sprintf( strTemp, 128, "*exhaust%i", i + 1 );
			ent->m_pVehicle->m_iExhaustTag[i] = gi.G2API_AddBolt( ent->ghoul2, 0, strTemp );
		}
		...
		for ( i = 0; i < MAX_VEHICLE_MUZZLES; i++ )
		{
			...
			ent->m_pVehicle->m_iMuzzleTag[i] = gi.G2API_AddBolt( ent->ghoul2, 0, strTemp );
		...
		for ( i = 0; i < MAX_VEHICLE_TURRET_MUZZLES; i++ )
		{
			...
			{
				ent->m_pVehicle->m_iGunnerViewTag[i] = gi.G2API_AddBolt( ent->ghoul2, 0, ent->m_pVehicle->m_pVehicleInfo->turret[i].gunnerViewTag );
			}
			else
			{
	...
	{ //a player or NPC saber user
		gi.G2API_AddBolt(ent->ghoul2, 0, "*r_hand");
		gi.G2API_AddBolt(ent->ghoul2, 0, "*l_hand");

		//rhand must always be first bolt. lhand always second. Whichever you want the
		//jetpack bolted to must always be third.
		gi.G2API_AddBolt(ent->ghoul2, 0, "*chestg");

		//claw bolts
		gi.G2API_AddBolt(ent->ghoul2, 0, "*r_hand_cap_r_arm");
		gi.G2API_AddBolt(ent->ghoul2, 0, "*l_hand_cap_l_arm");

		gi.G2API_SetBoneAnim(ent->ghoul2, 0, "model_root", 0, 12, BONE_ANIM_OVERRIDE_LOOP, 1.0f, level.time, -1, -1);
		gi.G2API_SetBoneAngles(ent->ghoul2, 0, "upper_lumbar", tempVec, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, NULL, 0, level.time);
		gi.G2API_SetBoneAngles(ent->ghoul2, 0, "cranium", tempVec, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_Y, POSITIVE_X, NULL, 0, level.time);

		if (!g2SaberInstance)
		{
			gi.G2API_InitGhoul2Model(&g2SaberInstance, "models/weapons2/saber/saber_w.glm", 0, 0, -20, 0, 0);

			if (g2SaberInstance)
			{
				// indicate we will be bolted to model 0 (ie the player) on bolt 0 (always the right hand) when we get copied
				gi.G2API_SetBoltInfo(g2SaberInstance, 0, 0);
				// now set up the gun bolt on it
				gi.G2API_AddBolt(g2SaberInstance, 0, "*blade1");
			}
		}
		...
		if (G_SaberModelSetup(ent))
		{
			if (g2SaberInstance)
			{
				gi.G2API_CopySpecificGhoul2Model(g2SaberInstance, 0, ent->ghoul2, 1); 
			}
		}
	}
	...
	{ //some extra NPC stuff
		if (gi.G2API_AddBolt(ent->ghoul2, 0, "lower_lumbar") == -1)
...
void Svcmd_ToggleUserinfoValidation_f( void ) {
	if ( gi.Argc() == 1 ) {
		...
		for ( i=0; i<numUserinfoFields; i++ ) {
			if ( (g_userinfoValidate.integer & (1<<i)) )	gi.Print( "%2d [X] %s\n", i, userinfoFields[i].fieldClean );
			else											gi.Print( "%2d [ ] %s\n", i, userinfoFields[i].fieldClean );
		}
		...
		gi.Cvar_Set( "g_userinfoValidate", va( "%i", (1<<index) ^ g_userinfoValidate.integer ) );
		gi.Cvar_Update( &g_userinfoValidate );
...
qboolean ClientUserinfoChanged( int clientNum ) {
	...
	if ( s && *s )
	{
		...
		gi.DropClient( clientNum, va( "Failed userinfo validation: %s", s ) );
		return qfalse;
	}
	...
	if ( client->pers.connected == CON_CONNECTED )
	{
		if ( strcmp( oldname, client->pers.netname ) ) 
		{
			...
			{				
				gi.SendServerCommand( -1, va( "print \"%s"S_COLOR_WHITE" %s %s\n\"", oldname, G_GetStringEdString( "MP_SVGAME", "PLRENAME" ), client->pers.netname ) );
...
//	teamTask = atoi(Info_ValueForKey(userinfo, "teamtask"));
	...
	gi.GetConfigstring( CS_PLAYERS+clientNum, oldClientinfo, sizeof( oldClientinfo ) );
...
char *ClientConnect( int clientNum, qboolean firstTime, qboolean isBot ) {
	...
	{ //if this is the first time then auto-assign a desired siege team and show briefing for that team
		...
		/*
		gi.SendServerCommand(ent->s.number, va("sb %i", client->sess.siegeDesiredTeam));
		*/
...
//	if ( !client->areabits )
//		client->areabits = G_Alloc( (gi.AAS_PointReachabilityAreaIndex( NULL ) + 7) / 8 );
...
void ClientBegin( int clientNum, qboolean allowTeamReset ) {
	...
	if ((ent->r.svFlags & SVF_BOT) && level.gametype >= GT_TEAM)
	{
		if (allowTeamReset)
		{
			...
			gi.GetUserinfo(clientNum, userinfo, MAX_INFO_STRING);
...
void G_UpdateClientAnims(gentity_t *self, float animSpeedScale)
{
	...
	{
		gi.G2API_SetBoneAnim(self->ghoul2, 0, "model_root", self->client->ps.saberLockFrame, self->client->ps.saberLockFrame+1, BONE_ANIM_OVERRIDE_FREEZE|BONE_ANIM_BLEND, animSpeedScale, level.time, -1, 150);
		gi.G2API_SetBoneAnim(self->ghoul2, 0, "lower_lumbar", self->client->ps.saberLockFrame, self->client->ps.saberLockFrame+1, BONE_ANIM_OVERRIDE_FREEZE|BONE_ANIM_BLEND, animSpeedScale, level.time, -1, 150);
		gi.G2API_SetBoneAnim(self->ghoul2, 0, "Motion", self->client->ps.saberLockFrame, self->client->ps.saberLockFrame+1, BONE_ANIM_OVERRIDE_FREEZE|BONE_ANIM_BLEND, animSpeedScale, level.time, -1, 150);
		return;
	}
	...
	if (self->client->legsAnimExecute != legsAnim || self->client->legsLastFlip != self->client->ps.legsFlip)
	{
		...
		gi.G2API_SetBoneAnim(self->ghoul2, 0, "model_root", firstFrame, lastFrame, aFlags, lAnimSpeedScale, level.time, -1, 150);
...
tryTorso:
		...
		!self->noLumbar)
	{
		...
		gi.G2API_SetBoneAnim(self->ghoul2, 0, "lower_lumbar", firstFrame, lastFrame, aFlags, lAnimSpeedScale, level.time, /*firstFrame why was it this before?*/-1, 150);
	...
	{ //only set the motion bone for humanoids.
		gi.G2API_SetBoneAnim(self->ghoul2, 0, "Motion", firstFrame, lastFrame, aFlags, lAnimSpeedScale, level.time, -1, 150);
	}
		...
		setTorso)
	{
		...
		{ //broken left arm
			...
			gi.G2API_SetBoneAnim(self->ghoul2, 0, brokenBone, armFirstFrame, armLastFrame, armFlags, armAnimSpeed, level.time, -1, 150);
		}
		...
		{ //broken right arm
				...
				(!self->client->saber[1].model[0] || self->client->ps.weapon != WP_SABER))
			{
				...
				{ //we want to keep the broken bone updated for some cases
					gi.G2API_SetBoneAnim(self->ghoul2, 0, brokenBone, firstFrame, lastFrame, aFlags, lAnimSpeedScale, level.time, -1, 150);
				}
					...
					(self->client->ps.torsoAnim == TORSO_WEAPONREADY2 || self->client->ps.torsoAnim == BOTH_ATTACK2 || self->client->ps.weapon < WP_BRYAR_PISTOL))
				{
					...
					gi.G2API_SetBoneAnim(self->ghoul2, 0, supportBone, armFirstFrame, armLastFrame, armFlags, armAnimSpeed, level.time, -1, 150);
				}
				else
				{ //we want to keep the support bone updated for some cases
					gi.G2API_SetBoneAnim(self->ghoul2, 0, supportBone, firstFrame, lastFrame, aFlags, lAnimSpeedScale, level.time, -1, 150);
				}
			}
			else
		...
		{ //remove the bone now so it can be set again
			...
			{ //can only have one arm broken at once.
				...
				gi.G2API_SetBoneAnim(self->ghoul2, 0, "lhumerus", 0, 1, 0, 0, level.time, -1, 0);
				gi.G2API_RemoveBone(self->ghoul2, "lhumerus", 0);
			}

			assert(brokenBone);

			//Set the flags and stuff to 0, so that the remove will succeed
			gi.G2API_SetBoneAnim(self->ghoul2, 0, brokenBone, 0, 1, 0, 0, level.time, -1, 0);

			//Now remove it
			gi.G2API_RemoveBone(self->ghoul2, brokenBone, 0);
...
*/
	...
	*/
	...
	gi.ICARUS_FreeEnt( (sharedEntity_t *)ent );
	gi.ICARUS_InitEnt( (sharedEntity_t *)ent );
}
	...
	if ( level.teamVoteTime[voteteam] ) {
		if ( ent->client->mGameFlags & PSG_TEAMVOTED ) {
			if ( ent->client->pers.teamvote == 1 ) {
				...
				gi.SetConfigstring( CS_TEAMVOTE_YES, va( "%i", level.teamVoteYes[voteteam] ) );
			}
			else if ( ent->client->pers.teamvote == 2 ) {
				level.teamVoteNo[voteteam]--;
				gi.SetConfigstring( CS_TEAMVOTE_NO, va( "%i", level.teamVoteNo[voteteam] ) );
			}
		}
...
void ClientDisconnect( int clientNum ) {
	...
	while (i < MAX_SABERS)
	{
		if (ent->client->weaponGhoul2[i] && gi.G2API_HaveWeGhoul2Models(ent->client->weaponGhoul2[i]))
		{
			gi.G2API_CleanGhoul2Models(&ent->client->weaponGhoul2[i]);
		}
		i++;
