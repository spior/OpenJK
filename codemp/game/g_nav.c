...
void NPC_Blocked( gentity_t *self, gentity_t *blocker )
{
	...
	//Debugi.Print( d_npcai, DEBUG_LEVEL_WARNING, "%s: Excuse me, %s %s!\n", self->targetname, blocker->classname, blocker->targetname );
...
void NPC_SetMoveGoal( gentity_t *ent, vec3_t point, int radius, qboolean isNavGoal, int combatPoint, gentity_t *targetEnt )
{
	...
	gi.LinkEntity( (sharedEntity_t *)ent->NPC->goalEntity );
}
...
//	return NAV_CheckAhead( self, point, trace, clipmask|CONTENTS_BOTCLIP );
	...
	if ( !gi.InPVS( self->r.currentOrigin, point ) )
		return qfalse;
	...
	if ( self->flags & FL_NAVGOAL )
	{
		...
		gi.Trace( &trace, point, mins, maxs, self->r.currentOrigin, self->parent->s.number, (clipmask|CONTENTS_MONSTERCLIP|CONTENTS_BOTCLIP)&~CONTENTS_BODY, qfalse, 0, 0 );
		if ( trace.startsolid&&(trace.contents&CONTENTS_BOTCLIP) )
		{//started inside do not enter, so ignore them
			clipmask &= ~CONTENTS_BOTCLIP;
			gi.Trace( &trace, point, mins, maxs, self->r.currentOrigin, self->parent->s.number, (clipmask|CONTENTS_MONSTERCLIP)&~CONTENTS_BODY, qfalse, 0, 0 );
		}
	...
	{
		gi.Trace( &trace, self->r.currentOrigin, mins, maxs, point, self->s.number, clipmask|CONTENTS_MONSTERCLIP|CONTENTS_BOTCLIP, qfalse, 0, 0);
		if ( trace.startsolid&&(trace.contents&CONTENTS_BOTCLIP) )
		{//started inside do not enter, so ignore them
			clipmask &= ~CONTENTS_BOTCLIP;
			gi.Trace( &trace, self->r.currentOrigin, mins, maxs, point, self->s.number, clipmask|CONTENTS_MONSTERCLIP, qfalse, 0, 0);
		}
...
int NAV_FindClosestWaypointForEnt( gentity_t *ent, int targWp )
{
	...
	return gi.Nav_GetNearestNode( (sharedEntity_t *)ent, ent->waypoint, NF_CLEAR_PATH, targWp );
}
...
int NAV_FindClosestWaypointForPoint( gentity_t *ent, vec3_t point )
{
	...
	bestWP = gi.Nav_GetNearestNode( (sharedEntity_t *)marker, marker->waypoint, NF_CLEAR_PATH, WAYPOINT_NONE );
...
qboolean NAV_CheckAhead( gentity_t *self, vec3_t end, trace_t *trace, int clipmask )
{
	...
	gi.Trace( trace, self->r.currentOrigin, mins, self->r.maxs, end, self->s.number, clipmask, qfalse, 0, 0 );
...
qboolean NAV_StackedCanyon( gentity_t *self, gentity_t *blocker, vec3_t pathDir )
{
	...
	gi.Trace( &tr, test, self->r.mins, self->r.maxs, test, self->s.number, self->clipmask|extraClip, qfalse, 0, 0 );
...
{//check only against architectrure
	...
	gi.Nav_GetNodePosition( endID, end );
	...
	if ( trace.entityNum < ENTITYNUM_WORLD )
	{
		...
		{//special case: doors are architecture, but are dynamic, like entitites
			...
			{//a locked door!
				...
				if ( failEdge )
				{
					gi.Nav_AddFailedEdge( self->s.number, startID, endID );
				}
			}
...
{
	return gi.Nav_GetNearestNode( (sharedEntity_t *)self, lastNode, NF_CLEAR_PATH, WAYPOINT_NONE );
}
...
int	NAV_MoveToGoal( gentity_t *self, navInfo_t *info )
{
	...
	bestNode = gi.Nav_GetBestNode( self->waypoint, self->NPC->goalEntity->waypoint, NODE_NONE );
	...
	if ( bestNode == WAYPOINT_NONE )
	{
		if ( NAVDEBUG_showEnemyPath )
		{
			...
			gi.Nav_GetNodePosition( self->NPC->goalEntity->waypoint, torigin );
			gi.Nav_GetNodePosition( self->waypoint, neworigin );
	...
	gi.Nav_GetNodePosition( self->waypoint, end );
	...
	if ( NAVDEBUG_showEnemyPath )
	{
		...
		gi.Nav_ShowPath( self->waypoint, self->NPC->goalEntity->waypoint );
	}
...
//	VectorMA( origin, MAX_RADIUS_CHECK, trace_dir, test_pos );
	...
	gi.Trace( &tr, origin, mins, maxs, test_pos, ENTITYNUM_NONE, ( CONTENTS_SOLID | CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP ), qfalse, 0, 0 );
...
void SP_waypoint ( gentity_t *ent )
{
	if ( navCalculatePaths )
	{
		...
		ent->health = gi.Nav_AddRawPoint( ent->r.currentOrigin, ent->spawnflags, radius );
...
void SP_waypoint_small (gentity_t *ent)
{
	if ( navCalculatePaths )
	{
		...
		ent->health = gi.Nav_AddRawPoint( ent->r.currentOrigin, ent->spawnflags, 2 );
		NAV_StoreWaypoint( ent );
		G_FreeEntity(ent);
		return;
	}
...
void Svcmd_Nav_f( void )
{
	...
	else if ( Q_stricmp( cmd, "set" ) == 0 )
	{
		...
		{
			NAVDEBUG_curGoal = gi.Nav_GetNearestNode( (sharedEntity_t *)&g_entities[0], g_entities[0].waypoint, NF_CLEAR_PATH, WAYPOINT_NONE );
		}
	}
...
void NAV_StoreWaypoint( gentity_t *ent )
{
	...
	if ( !tempWaypointList )
	{
		...
		gi.TrueMalloc((void **)&tempWaypointList, size);
	...
	*/
	...
	{
		//gi.Error( ERR_DROP, "Too many waypoints!  (%d > %d)\n", numStoredWaypoints, MAX_STORED_WAYPOINTS );
...
#endif
			{
				gi.Nav_HardConnect( tempWaypointList[i].nodeID, tempWaypointList[target].nodeID );
			}
		}
...
void NAV_ShowDebugInfo( void )
{
	...
	if ( NAVDEBUG_showTestPath )
	{
		...
		int	nearestNode = gi.Nav_GetNearestNode( (sharedEntity_t *)&g_entities[0], g_entities[0].waypoint, NF_ANY, WAYPOINT_NONE );
		int	testNode = gi.Nav_GetBestNode( nearestNode, NAVDEBUG_curGoal, NODE_NONE );
		vec3_t	dest, start;
		...
		//Get the positions
		gi.Nav_GetNodePosition( NAVDEBUG_curGoal, dest );
		gi.Nav_GetNodePosition( nearestNode, start );

		G_DrawNode( start, NODE_START );
		G_DrawNode( dest, NODE_GOAL );
		gi.Nav_ShowPath( nearestNode, NAVDEBUG_curGoal );
	}
...
{
	g_entities[clNum].waypoint = gi.Nav_GetNearestNode( (sharedEntity_t *)&g_entities[clNum], g_entities[clNum].lastWaypoint, NF_CLEAR_PATH, WAYPOINT_NONE );
}
