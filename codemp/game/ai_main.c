...
void BotReportStatus(bot_state_t *bs)
{
	...
	{
		gi.EA_SayTeam(bs->client, teamplayStateDescriptions[bs->teamplayState]);
	}
	else if (level.gametype == GT_SIEGE)
	{
		gi.EA_SayTeam(bs->client, siegeStateDescriptions[bs->siegeState]);
	}
	else if (level.gametype == GT_CTF || level.gametype == GT_CTY)
	{
		gi.EA_SayTeam(bs->client, ctfStateDescriptions[bs->ctfState]);
	}
}
...
int BotAI(int client, float thinktime) {
	...
	while( gi.BotGetServerCommand(client, buf, sizeof(buf)) ) {
	...
	gi.Cvar_Update(&bot_debugmessages);
...
int OrgVisible(vec3_t org1, vec3_t org2, int ignore)
{
	...
	gi.Trace(&tr, org1, NULL, NULL, org2, ignore, MASK_SOLID, qfalse, 0, 0 );
...
int WPOrgVisible(gentity_t *bot, vec3_t org1, vec3_t org2, int ignore)
{
	...
	if (tr.fraction == 1)
	{
		gi.Trace(&tr, org1, NULL, NULL, org2, ignore, MASK_PLAYERSOLID, qfalse, 0, 0);
...
int OrgVisibleBox(vec3_t org1, vec3_t mins, vec3_t maxs, vec3_t org2, int ignore)
{
	...
	{
		gi.Trace(&tr, org1, mins, maxs, org2, ignore, MASK_SOLID, qfalse, 0, 0);
	}
...
int CheckForFunc(vec3_t org, int ignore)
{
	...
	gi.Trace(&tr, org, NULL, NULL, under, ignore, MASK_SOLID, qfalse, 0, 0);
...
int BotTrace_Strafe(bot_state_t *bs, vec3_t traceto)
{
	...
	gi.Trace(&tr, from, playerMins, playerMaxs, to, bs->client, MASK_PLAYERSOLID, qfalse, 0, 0);
...
int BotTrace_Jump(bot_state_t *bs, vec3_t traceto)
{
	...
	gi.Trace(&tr, bs->origin, mins, maxs, traceto_mod, bs->client, MASK_PLAYERSOLID, qfalse, 0, 0);
	...
	gi.Trace(&tr, tracefrom_mod, mins, maxs, traceto_mod, bs->client, MASK_PLAYERSOLID, qfalse, 0, 0);
	...
	while (i < level.num_entities)
	{
		...
		{ //try to escape from anything with a non-0 s.weapon and non-0 damage. This hopefully only means dangerous projectiles.
		  //Or a sentry gun if bolt_Head == 1000. This is a terrible hack, yes.
			...
			{
				gi.Trace(&tr, bs->origin, NULL, NULL, ent->s.pos.trBase, bs->client, MASK_SOLID, qfalse, 0, 0);
...
{ //get the nearest possible waypoint to the flag since it's not in its original position
	...
	{
		gi.Trace(&tr, wp->origin, mins, maxs, flagEnt->s.pos.trBase, flagEnt->s.number, MASK_SOLID, qfalse, 0, 0);
	...
	while (i < gWPNum)
	{
		...
		{
			gi.Trace(&tr, gWPArray[i]->origin, mins, maxs, flagEnt->s.pos.trBase, flagEnt->s.number, MASK_SOLID, qfalse, 0, 0);
...
hasPoint:
	...
	if (bs->touchGoal)
	{
		//gi.Print("Please, master, let me touch it!\n");
	...
	if (bs->siegeState == SIEGESTATE_ATTACKER)
	{
		...
		{ //looks like we have no goals other than to keep the other team from completing objectives
			...
			if (bs->shootGoal)
			{
				...
				{
					gi.Trace(&tr, bs->origin, NULL, NULL, dif, bs->client, MASK_SOLID, qfalse, 0, 0);
...
void GetIdealDestination(bot_state_t *bs)
{
	...
	if (badthing)
	{
		...
		if (gWPArray[tempInt] && gWPArray[tempInt]->inuse && bs->escapeDirTime < level.time)
		{
			...
			if (plusLen < minusLen)
			{
				...
				//gi.Print("Escaping from scary bad thing [%s]\n", badthing->classname);
			}
		}
		//gi.Print("Run away run away run away!\n");
		return;
	}
	...
	{
		//gi.Print("I need something to do\n");
		idleWP = GetBestIdleGoal(bs);
...
void MeleeCombatHandling(bot_state_t *bs)
{
	...
	gi.Trace(&tr, usethisvec, mins, maxs, downvec, -1, MASK_SOLID, qfalse, 0, 0);
	...
	gi.Trace(&tr, bs->origin, mins, maxs, downvec, -1, MASK_SOLID, qfalse, 0, 0);
	...
	gi.Trace(&tr, midorg, mins, maxs, downvec, -1, MASK_SOLID, qfalse, 0, 0);
		...
		if (bs->currentEnemy && bs->currentEnemy->client)
		{
			...
			else if (bs->currentEnemy->client->ps.weapon == WP_SABER && bs->frame_Enemy_Len < 80 && ((Q_irand(1, 10) < 8 && bs->saberBFTime < level.time) || bs->saberBTime > level.time || BG_SaberInKata(bs->currentEnemy->client->ps.saberMove) || bs->currentEnemy->client->ps.saberMove == LS_SPINATTACK || bs->currentEnemy->client->ps.saberMove == LS_SPINATTACK_DUAL))
			{
				...
				while (checkIncr < idealDist)
				{
					...
					gi.Trace(&tr, bs->goalPosition, NULL, NULL, groundcheck, bs->client, MASK_SOLID, qfalse, 0, 0);
...
void BotAimLeading(bot_state_t *bs, vec3_t headlevel, float leadAmount)
{
	...
	//gi.Print("Leadin target with a velocity total of %f\n", vtotal);
...
int BotFallbackNavigation(bot_state_t *bs)
{
	...
	gi.Trace(&tr, bs->origin, mins, maxs, trto, ENTITYNUM_NONE, MASK_SOLID, qfalse, 0, 0);
...
void StrafeTracing(bot_state_t *bs)
{
	...
	gi.Trace(&tr, bs->origin, mins, maxs, rorg, bs->client, MASK_SOLID, qfalse, 0, 0);
	...
	gi.Trace(&tr, rorg, NULL, NULL, drorg, bs->client, MASK_SOLID, qfalse, 0, 0);
...
gentity_t *CheckForFriendInLOF(bot_state_t *bs)
{
	...
	gi.Trace(&tr, trfrom, mins, maxs, trto, bs->client, MASK_PLAYERSOLID, qfalse, 0, 0);
	...
	if (flagRed && flagBlue)
	{
			...
			bs->wpDestination == flagBlue)
		{
			...
			if (diddrop && desiredDrop)
			{
				...
				if (VectorLength(a) <= BOT_FLAG_GET_DISTANCE)
				{
					gi.Trace(&tr, bs->origin, mins, maxs, desiredDrop->s.pos.trBase, bs->client, MASK_SOLID, qfalse, 0, 0);
...
int BotSurfaceNear(bot_state_t *bs)
{
	...
	gi.Trace(&tr, bs->origin, NULL, NULL, fwd, bs->client, MASK_SOLID, qfalse, 0, 0);
...
void StandardBotAI(bot_state_t *bs, float thinktime)
{
	...
	{ //stupid vehicle debug, I tire of having to connect another client to test passengers.
		...
		{ //in a vehicle, so...
			...
			{
				gi.EA_MoveForward(bs->client);

				if (bot_getinthecarrr.integer == 3)
				{ //use alt fire
					gi.EA_Alt_Attack(bs->client);
				}
			}
		}
		else
		{ //find one, get in
			...
			{ //broke before end so we must've found something
				...
				gi.EA_Move(bs->client, v, 5000.0f);
	...
	if (bot_forgimmick.integer)
	{
		...
		{ //for testing cpu usage moving around rmg terrain without AI
			...
			gi.EA_Attack(bs->client);
			gi.EA_Move(bs->client, mdir, 5000);
		}
		...
		if (bs->forceMove_Right)
		{
			...
			{
				gi.EA_MoveRight(bs->client);
			}
			else
			{
				gi.EA_MoveLeft(bs->client);
			}
		}
		if (bs->forceMove_Up)
		{
			gi.EA_Jump(bs->client);
		}
		return;
	}
	...
	else if (bs->doChat && bs->chatTime <= level.time)
	{
		if (bs->chatTeam)
		{
			gi.EA_SayTeam(bs->client, bs->currentChat);
			bs->chatTeam = 0;
		}
		else
		{
			gi.EA_Say(bs->client, bs->currentChat);
		}
		if (bs->doChat == 2)
		{
	...
	if (bs->beStill < level.time && !WaitingForNow(bs, bs->goalPosition) && !fjHalt)
	{
		...
		{
		//	gi.EA_Move(bs->client, bs->origin, 5000);
			bs->beStill = level.time + 200;
		}
		else
		{
			gi.EA_Move(bs->client, bs->goalMovedir, 5000);
		}
...
//	static int botlib_residual;
	...
	{
		gi.Cvar_Update(&bot_pvstype);
		gi.Cvar_Update(&bot_camp);
		gi.Cvar_Update(&bot_attachments);
		gi.Cvar_Update(&bot_forgimmick);
		gi.Cvar_Update(&bot_honorableduelacceptance);
#ifndef FINAL_BUILD
		gi.Cvar_Update(&bot_getinthecarrr);
#endif
...
int BotAISetup( int restart ) {
	//rww - new bot cvars..
	gi.Cvar_Register(&bot_forcepowers, "bot_forcepowers", "1", CVAR_CHEAT);
	gi.Cvar_Register(&bot_forgimmick, "bot_forgimmick", "0", CVAR_CHEAT);
	gi.Cvar_Register(&bot_honorableduelacceptance, "bot_honorableduelacceptance", "0", CVAR_CHEAT);
	gi.Cvar_Register(&bot_pvstype, "bot_pvstype", "1", CVAR_CHEAT);
#ifndef FINAL_BUILD
	gi.Cvar_Register(&bot_getinthecarrr, "bot_getinthecarrr", "0", 0);
#endif

#ifdef _DEBUG
	gi.Cvar_Register(&bot_nogoals, "bot_nogoals", "0", CVAR_CHEAT);
	gi.Cvar_Register(&bot_debugmessages, "bot_debugmessages", "0", CVAR_CHEAT);
#endif

	gi.Cvar_Register(&bot_attachments, "bot_attachments", "1", 0);
	gi.Cvar_Register(&bot_camp, "bot_camp", "1", 0);

	gi.Cvar_Register(&bot_wp_info, "bot_wp_info", "1", 0);
	gi.Cvar_Register(&bot_wp_edit, "bot_wp_edit", "0", CVAR_CHEAT);
	gi.Cvar_Register(&bot_wp_clearweight, "bot_wp_clearweight", "1", 0);
	gi.Cvar_Register(&bot_wp_distconnect, "bot_wp_distconnect", "1", 0);
	gi.Cvar_Register(&bot_wp_visconnect, "bot_wp_visconnect", "1", 0);

	gi.Cvar_Update(&bot_forcepowers);
	//end rww
