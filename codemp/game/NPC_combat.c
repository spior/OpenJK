...
qboolean ShotThroughGlass (trace_t *tr, gentity_t *target, vec3_t spot, int mask)
{
	...
	{//ok to shoot through breakable glass
		...
		gi.Trace (tr, muzzle, NULL, NULL, spot, skip, mask, qfalse, 0, 0 );
		return qtrue;
	}
...
qboolean CanShoot ( gentity_t *ent, gentity_t *shooter ) 
{
	...
	gi.Trace ( &tr, muzzle, NULL, NULL, spot, shooter->s.number, MASK_SHOT, qfalse, 0, 0 );
			...
			if( newenemy->health > 0 )
			{
				...
				{//FIXME:  check for range and FOV or vis?
					...
					{//Make sure we're not just going back and forth here
						if ( gi.InPVS(newenemy->r.currentOrigin, NPCS.NPC->r.currentOrigin) )
						{
		...
		if ( ally->client )
		{
			if ( ally->health > 0 )
			{
				...
				{//if on same team or if player is disguised as your team
					...
					if(!gi.InPVS(ally->r.currentOrigin, NPCS.NPC->r.currentOrigin))
					{
						continue;
					}
...
gentity_t *NPC_CheckEnemy( qboolean findNew, qboolean tooFarOk, qboolean setEnemy )
{
	...
	*/
	...
	if ( NPCS.NPC->enemy )
	{
		...
		else if ( !gi.InPVS(NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.currentOrigin ) )
...
//	if ( NPC->client->playerTeam == TEAM_SCAVENGERS )
	if( NPCS.NPC->s.weapon == WP_BLASTER /*|| NPC->s.weapon == WP_BLASTER_PISTOL*/ ) // any other guns to check for?
	{
		...
		gi.Trace ( &tr, muzzle, mins, maxs, ent->r.currentOrigin, NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0 );
	}
	else
	{
		gi.Trace ( &tr, muzzle, NULL, NULL, ent->r.currentOrigin, NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0 );
	}
...
//	if ( NPC->client->playerTeam == TEAM_SCAVENGERS )
	if( NPCS.NPC->s.weapon == WP_BLASTER /*|| NPC->s.weapon == WP_BLASTER_PISTOL*/ ) // any other guns to check for?
	{
		...
		gi.Trace ( &tr, muzzle, mins, maxs, targ, NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0 );
	}
	else
	{
		gi.Trace ( &tr, muzzle, NULL, NULL, targ, NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0 );
	}
...
//	qboolean	duck_ok = qfalse;
	...
	{//He's in our FOV
		...
		if(attack_ok)
		{
			...
			gi.Trace( &tr, muzzle, NULL, NULL, hitspot, NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0 );
...
int NPC_FindCombatPoint( const vec3_t position, const vec3_t avoidPosition, vec3_t enemyPosition, const int flags, const float avoidDist, const int ignorePoint )
{
	...
	for ( j = 0; j < numPoints; j++ )
	{
		...
		if ( flags & CP_AVOID_ENEMY  )
		{
			...
			{
				gi.Nav_GetNodePosition( NPC->waypoint, wpOrg );
			}
			else
			*/
			{
		...
		gi.Trace( &tr, level.combatPoints[i].origin, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, level.combatPoints[i].origin, NPCS.NPC->s.number, NPCS.NPC->clipmask, qfalse, 0, 0 );
		...
		if ( flags & CP_HAS_ROUTE )
		{
			...
			*/
			...
			if ( waypoint == WAYPOINT_NONE || level.combatPoints[i].waypoint == WAYPOINT_NONE || gi.Nav_GetBestNodeAltRoute2( waypoint, level.combatPoints[i].waypoint, NODE_NONE ) == WAYPOINT_NONE )
		...
		{
			cost = gi.Nav_GetPathCost( waypoint, level.combatPoints[i].waypoint );
			if ( cost < bestCost )
			{
				bestCost = cost;
				best = i;
...
//		}
		...
		{
			if ( gi.InPVS( found->r.currentOrigin, NPCS.NPC->r.currentOrigin ) )
			{
				dist = DistanceSquared( found->r.currentOrigin, NPCS.NPC->r.currentOrigin );
				if ( dist < bestDist )
				{
					if ( !gi.Nav_GetBestPathBetweenEnts( (sharedEntity_t *)NPCS.NPC, (sharedEntity_t *)found, NF_CLEAR_PATH ) 
						|| gi.Nav_GetBestNodeAltRoute2( NPCS.NPC->waypoint, found->waypoint, NODE_NONE ) == WAYPOINT_NONE )
					{//can't possibly have a route to any OR can't possibly have a route to this one OR don't have a route to this one
...
void NPC_CheckGetNewWeapon( void )
{
	...
	{//if running away because dropped weapon...
		...
		{//need a weapon, any lying around?
			...
			{//try to nav to it
				/*
				if ( !gi.Nav_GetBestPathBetweenEnts( NPC, foundWeap, NF_CLEAR_PATH ) 
					|| gi.Nav_GetBestNodeAltRoute( NPC->waypoint, foundWeap->waypoint ) == WAYPOINT_NONE )
				{//can't possibly have a route to any OR can't possibly have a route to this one OR don't have a route to this one
