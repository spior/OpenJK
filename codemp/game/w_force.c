...
void WP_InitForcePowers( gentity_t *ent )
{
	...
	{//ack, prevent user from being dumb
		maxRank = FORCE_MASTERY_JEDI_MASTER;
		gi.Cvar_Set( "g_maxForceRank", va("%i", maxRank) );
	}
	...
	*/
	...
	{ //Then use the powers for this class, and skip all this nonsense.
		...
		if (!ent->client->sess.setForce)
		{
			...
			gi.SendServerCommand(ent-g_entities, "scl");
		}
	...
	if ( strlen( forcePowers ) < strlen( DEFAULT_FORCEPOWERS ) )
	{
		...
		gi.SendServerCommand( ent-g_entities, "print \"^1Invalid forcepowers string, setting default\n\"" );
	}
...
//				if (!(ent->r.svFlags & SVF_BOT) && level.gametype != GT_DUEL && level.gametype != GT_POWERDUEL && ent->s.eType != ET_NPC)
				if (!(ent->r.svFlags & SVF_BOT) && ent->s.eType != ET_NPC)
				{
					if (!g_teamAutoJoin.integer)
					{
						...
						gi.SendServerCommand(ent-g_entities, "spc");	// Fire up the profile menu
					}
				}
				...
				gi.SendServerCommand(ent->s.number, va("nfr %i %i %i", maxRank, 1, ent->client->sess.sessionTeam));
		...
		if (!didEvent )
		{
			...
			te->s.bolt2 = ent->client->sess.sessionTeam;
#else
			gi.SendServerCommand(ent->s.number, va("nfr %i %i %i", maxRank, 0, ent->client->sess.sessionTeam));
#endif
		}

		if (warnClientLimit)
		{ //the server has one or more force powers disabled and the client is using them in his config
			//gi.SendServerCommand(ent-g_entities, va("print \"The server has one or more force powers that you have chosen disabled.\nYou will not be able to use the disable force power(s) while playing on this server.\n\""));
		}
	}
...
void ForceTeamHeal( gentity_t *self )
{
	...
	while (i < MAX_CLIENTS)
	{
		...
		if (ent && ent->client && self != ent && OnSameTeam(self, ent) && ent->client->ps.stats[STAT_HEALTH] < ent->client->ps.stats[STAT_MAX_HEALTH] && ent->client->ps.stats[STAT_HEALTH] > 0 && ForcePowerUsableOn(self, ent, FP_TEAM_HEAL) &&
			gi.InPVS(self->client->ps.origin, ent->client->ps.origin))
		{
...
void ForceGrip( gentity_t *self )
{
	...
	gi.Trace(&tr, tfrom, NULL, NULL, tto, self->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);
...
void ForceShootLightning( gentity_t *self )
{
	...
	{//arc
		...
		numListedEntities = gi.EntitiesInBox( mins, maxs, iEntityList, MAX_GENTITIES );
		...
		for ( e = 0 ; e < numListedEntities ; e++ ) 
		{
			...
			//in PVS?
			if ( !traceEnt->r.bmodel && !gi.InPVS( ent_org, self->client->ps.origin ) )
			{//must be in PVS
				continue;
			}

			//Now check and see if we can actually hit it
			gi.Trace( &tr, self->client->ps.origin, vec3_origin, vec3_origin, ent_org, self->s.number, MASK_SHOT, qfalse, 0, 0 );
	...
	{//trace-line
		...
		gi.Trace( &tr, self->client->ps.origin, vec3_origin, vec3_origin, end, self->s.number, MASK_SHOT, qfalse, 0, 0 );
...
void ForceThrow( gentity_t *self, qboolean pull )
{
	...
	for ( e = 0 ; e < numListedEntities ; e++ ) 
	{
		...
		//in PVS?
		if ( !ent->r.bmodel && !gi.InPVS( ent_org, self->client->ps.origin ) )
		{//must be in PVS
			continue;
		}
		...
		{//must have clear LOS
			...
			gi.Trace( &tr, eyePoint, vec3_origin, vec3_origin, ent_org, self->s.number, MASK_SHOT, qfalse, 0, 0 );
	...
	if ( ent_count )
	{
		...
		for ( x = 0; x < ent_count; x++ )
		{
			...
			{//push/pull the door
				...
				gi.Trace( &tr, trFrom, vec3_origin, vec3_origin, end, self->s.number, MASK_SHOT, qfalse, 0, 0 );
...
void DoGripAction(gentity_t *self, forcePowers_t forcePower)
{
	...
	gi.Trace(&tr, self->client->ps.origin, NULL, NULL, gripEnt->client->ps.origin, self->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);
	...
	while (i < MAX_CLIENTS)
	{
		if (G_IsMindTricked(&self->client->ps.fd, i))
		{
			...
			{ //Untrick this entity if the tricker (self) fires while in his fov
				if (gi.InPVS(ent->client->ps.origin, self->client->ps.origin) &&
...
void SeekerDroneUpdate(gentity_t *self)
{
	...
	if (self->client->ps.genericEnemyIndex != ENTITYNUM_NONE && self->client->ps.genericEnemyIndex != -1)
	{
		...
		if (self->client->ps.droneFireTime < level.time)
		{
			gi.Trace(&tr, org, NULL, NULL, en->client->ps.origin, -1, MASK_SOLID, qfalse, 0, 0);
...
{ //keep holocron status updated in holocron mode
	...
	gi.Cvar_Update(&g_maxHolocronCarry);
